...
let ferraris = cars.filter( x => x.brand === 'Ferrari')

let nineties = cars.filter( x => x.model >= 1980 && x.model <= 1990 )

let list = cars.map( x => `This ${x.brand} costs ${x.price}`)

let sumBMW = cars.filter( x => x.brand === 'BMW').map( x => x.price).reduce((a,b) => a + b)
